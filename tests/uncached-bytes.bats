...
@test "uncached bytes should be 0 before the first build and should be 0 after a build" {
  ...
  run curl --silent "http://builder-0.builder.container-image-builder.svc.cluster.local:2375/uncachedBytes?cachefrom=%5B%5D&t=uncached-$DATE"
  [ "$status" -eq 0 ]
  [ "${lines[0]}" = "0" ]
  run curl --silent "http://builder-1.builder.container-image-builder.svc.cluster.local:2375/uncachedBytes?cachefrom=%5B%5D&t=uncached-$DATE"
  [ "$status" -eq 0 ]
  [ "${lines[0]}" = "0" ]
